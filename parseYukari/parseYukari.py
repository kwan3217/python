...
def dump(pkt):
    global base,pktdesc,apidfirst
    oufn = base + '/' + pktdesc[pkt['apid']]['name']
...
def addField(pkt):
    ...
    if pkt['descType'] == 0:
        if pkt['descApid'] in pktdesc:
            ...
            pktdesc[pkt['descApid']] = {'name': pkt['descName'], 'handler': csv if pkt['descName'] != 'Dump' else dump,
                                        'fields': OrderedDict()}
    else:
        pktdesc[pkt['descApid']]['fields'][pkt['descName']] = {'pos': pkt['descPos'], 'type': pkt['descType']}

#Preload 11 (bootstrap) and 12 (not complete)
yukaripktdesc = OrderedDict({11:{'name': 'PacketDescription',
                                 'handler': addField,
                             ...
                             12:{'name': 'CCSDS self-documentation',
                                 'handler': print,
                                 'fields': OrderedDict({'Documentation': {'pos': 6, 'type': t_string}})}
                             })
southwestpktdesc = OrderedDict({0x00C:{'name': 'Version',
                                       'handler': csv,
                                       'fields': OrderedDict({
                                                  'hw_type':   {'pos':  6, 'type': t_i32},
                                ...
                                0x022:{'name': 'ConfigFile',
                                       'handler': csv,
                                       'fields': OrderedDict({
                                                  'base': {'pos': 6 + 0x00, 'type': t_string, 'len': 8},
...
pocketpktdesc=OrderedDict({0x001:{'name': 'Version',
                        'handler': csv,
...
def parseYukari(infn,apid_blacklist=[0x009,0x00A,0x00C,0x00F,0x00E],skip=8):
    ...
    with open(infn,"rb") as inf:
        ...
        import time
        import os
        fsize=os.fstat(inf.fileno()).st_size
        while not done:
            if pktcount%1000000==0:
                perf=time.perf_counter()
                if lastproc is not None:
                    dperf=perf-lastperf
                    dproc=inf.tell()-lastproc
                    print(pktcount,inf.tell(),inf.tell()*100/fsize,dproc/dperf)
                else:
                    print(pktcount,inf.tell())
                lastproc = inf.tell()
                lastperf = perf
            pktcount+=1
            header=inf.read(6)
            ...
            if len(body)<pktlen:
                ...
                return
            pkt={'apid':apid}
            if apid not in apid_blacklist and apid in pktdesc:
