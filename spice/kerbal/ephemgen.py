...
ksp_planets = dict([i for i in ksp_bodies.items() if i[1].orbit != None and i[1].orbit.body.name == "Sun"])
ksp_planet_list = list(ksp_planets.values())
ksp_planet_list.sort(key=lambda x:x.orbit.semi_major_axis)

#Get all the moons, and put them into dictionaries by planet.
ksp_moons = dict([i for i in ksp_bodies.items() if i[1].orbit != None and i[1].orbit.body.name != "Sun"])
ksp_moons_by_planet = defaultdict(list)
for one_moon in ksp_moons.values():
	ksp_moons_by_planet[one_moon.orbit.body.name].append(one_moon)

#sort the moon lists
for one_list_of_moons in ksp_moons_by_planet.values():
	one_list_of_moons.sort(key=lambda x:x.orbit.semi_major_axis)

solar_gm=ksp_sun.gravitational_parameter*1e-9
